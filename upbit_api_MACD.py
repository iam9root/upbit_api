...
def Moving_Average_Line(response):
	...
	print("기간 합계, 기간 평균")
	last_price_sum = first_value['trade_price']
	for i in range(1,len(form)-1):
		value = "{"+form[i]
		value = literal_eval(value)
		last_price_sum += value['trade_price']

	for i in range(1,len(form)):
		value = "{"+form[i]
		value = literal_eval(value)
		pre_price_sum += value['trade_price']

		
	pre_price_average = (pre_price_sum / (len(form)-1))
	last_price_average = (last_price_sum / (len(form)-1))

	print(pre_price_sum, pre_price_average)

	return pre_price_average, last_price_average


def Candle30_Line15():
	url = "https://api.upbit.com/v1/candles/minutes/10"
	short_line = {"market":"KRW-BTC", "count":"4"}
	long_line = {"market":"KRW-BTC", "count":"7"}
	s_response = requests.get(url, params=short_line)
	l_response = requests.get(url, params=long_line)
	
	sh, lsh=Moving_Average_Line(s_response)
	lo, llo=Moving_Average_Line(l_response)
...
if __name__=="__main__":
	...
	file = open(desktop_path+'\\macd_original.csv', 'a', encoding='euc_kr', newline='')
	...
	while(1):
		...
		file.close()
		try:
			shutil.copy(desktop_path+'\\macd_original.csv', desktop_path+'\\copy.csv')
		except PermissionError as a:
			shutil.copy(desktop_path+'\\macd_original.csv', desktop_path+'\\copy2.csv')
		time.sleep(60)